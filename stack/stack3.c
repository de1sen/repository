...
typedef struct {           
    size_t size;     // сколько элементов хранится в стеке
    size_t capacity;        // ёмкость стека(для скольки элементов выделена память)
    Data a[1];          // данные    
} Stack;    
...
void push(Stack **pst, Data data)
{
    if (is_full(*pst)) {
        (*pst)->capacity += N;
        *pst  = realloc(*pst, sizeof(Stack) + (*pst)->capacity * sizeof(Data));    
    }
    Stack * st = *pst;
    st->a[st->size] = data;
    st->size ++;
